...
const Order = ({ userOrder, orderId }) => {
  return (
    <ContainerFather>
      <h1 className="text-sm md:text-3xl font-semibold uppercase p-4 text-black-500 md:mt-12 text-center ">
        ¡Gracias por su compra!
      </h1>
      <p className="text-sm md:text-3xl font-semibold uppercase text-black-500 mt-1 md:mt-2 text-center">
        ¡Esperamos verte por aquí nuevamente!
      </p>
      <OrderContainer>
        <MyUser>
          ...
          <ul>
            {userOrder.items.map((item) => (
              <li key={item.id} className="whitespace-nowrap">
                -{`${item.titulo} X ${item.quantity} U. ($${item.precio * item.quantity})`}
              </li>
            ))}
          </ul>
        </MyUser2>
